...
from ..utils.paths import get_logs_root, get_deepdata_home
...
def _get_fork_workspace() -> dict | None:
    """Build synthetic workspace for fork mode. Returns None if not in fork mode."""
    if os.getenv('AGENT_FORK_MODE') != '1':
        return None
    fork_session_id = os.getenv('AGENT_RESUME_SESSION_ID')
    if not fork_session_id:
        return None
    try:
        info = session_store.get_session_info(fork_session_id)
        session_name = info.session_name or "Agent"
    except Exception:
        session_name = "Agent"
    return {
        "agent_tabs": [{
            "tab_id": f"tab_{fork_session_id[:8]}",
...
async def get_workspace():
    ...
    In fork mode, returns a synthetic workspace with one tab for the forked session.
    """
    from .handlers.workspace import get_cwd_info

    workspace = _get_fork_workspace() or workspace_manager.load_workspace()
    return {"workspace": workspace, "cwd": get_cwd_info()}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        # Load workspace (fork mode uses synthetic workspace)
        from ..core.session_registry import get_session_store
        from .handlers.workspace import get_cwd_info
        session_store = get_session_store(get_logs_root())

        workspace = _get_fork_workspace()
...
_docs_dir = get_deepdata_home() / "docs"
