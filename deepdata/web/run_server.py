...
Examples:
  ...
  deepdata serve --mle --preset mlebench -t 3600 -n 3 -m opus
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="SESSION_ID",
        help="[Web] Fork (clone) a session and start from it"
    )
    ...
    if args.fork and (args.resume or args.continue_last):
        parser.error("Cannot use --fork with --resume or --continue")

    # Use current directory if --cwd not specified
    cwd = args.cwd or os.getcwd()

    # Handle --fork: clone session before starting server
    if args.fork:
        from ..core.session import SessionStore
        store = SessionStore()
        try:
            new_session_id = store.clone_session(args.fork)
        except ValueError as e:
            print(f"Error: {e}")
            sys.exit(1)
        print(f"\n  Forked: {args.fork[:8]}.. → {new_session_id[:8]}..")
        os.environ['AGENT_RESUME_SESSION_ID'] = new_session_id
        os.environ['AGENT_FORK_MODE'] = '1'
