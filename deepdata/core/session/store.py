...
class SessionStore(SessionQueriesMixin, SessionExportMixin):
    ...
    def _migrate_session_db(self, conn: sqlite3.Connection):
        ...
        # Migration: Recreate conversation_blocks to add 'thinking' to block_type CHECK
        try:
            row = conn.execute(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='conversation_blocks'"
            ).fetchone()
            if row and "'thinking'" not in row[0]:
                conn.executescript("""
                    CREATE TABLE IF NOT EXISTS conversation_blocks_new (
                    ...
                    INSERT INTO conversation_blocks_new (id, turn_number, block_index, timestamp, role, block_data)
                        SELECT id, turn_number, block_index, timestamp, role, block_data
                        FROM conversation_blocks;
                    DROP TABLE conversation_blocks;
                    ALTER TABLE conversation_blocks_new RENAME TO conversation_blocks;
                    ...
                    CREATE INDEX IF NOT EXISTS idx_tool_name
                        ON conversation_blocks(tool_name) WHERE tool_name IS NOT NULL;
                    CREATE INDEX IF NOT EXISTS idx_tool_use_id
                        ON conversation_blocks(tool_use_id) WHERE tool_use_id IS NOT NULL;
                """)
                conn.commit()
        except sqlite3.OperationalError:
